...
cmd_progthread(void *ptr, unsigned long nargs)
{
	...
	result = runprogram((char*)progname,(char**) args, (int)nargs);
...
/*
 * Command for viewing debug flags.
 */
static
int
cmd_dbflags(int nargs, char **args)
{
	...
	kprintf("OS/161 operations menu\n");
	//kprintf("IM HERE\n");
	...
	{
    		int test = dbflags;
    		res[0] = TO_HEX(test%10);  
		test = test/10;	 
    		res[1] = TO_HEX(test%10);
		test = test/10;
    		res[2] = TO_HEX(test%10);
    		res[3] = '\0';
	}
...
cmd_df(int nargs, char **args)
{
	...
	//dbflags = strtol(args[1], '\0', 16);
	//kprintf("Current value of dbflags is %ld\n", dbflags);
